...
void SpawnEntityHandler::invoke(openage::event::EventLoop & /* loop */,
        ...
        if (params.get<bool>("cycle", false)) {
                if (!test_entities.empty()) {
                        index = (index + 1) % test_entities.size();
                        log::log(MSG(info) << "Selected spawn unit: " << test_entities.at(index));
                }
                return;
        }
        ...
        // If "nyan_entity" param is provided (from IPC), use it directly.
        // Otherwise fall back to the test_entities cycling list.
        nyan::fqon_t nyan_entity;
        if (params.contains("nyan_entity")) {
                nyan_entity = params.get<std::string>("nyan_entity");
        }
        else {
                if (test_entities.empty()) {
                        return;
                }
                nyan_entity = test_entities.at(index);
        }
        ...
        // If a result promise was provided (IPC spawn), fulfill it with the entity ID
        if (params.contains("result_promise")) {
                auto promise = params.get<std::shared_ptr<std::promise<uint64_t>>>("result_promise");
                if (promise) {
                        promise->set_value(entity->get_id());
                }
        }
}
