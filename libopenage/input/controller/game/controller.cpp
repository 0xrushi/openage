...
void setup_defaults(const std::shared_ptr<BindingContext> &ctx,
        ...
        binding_func_t create_entity_event{[&](const event_arguments &args,
                ...
                // Log entity drop position for spawn_tool debugging
                log::log(MSG(info) << "Entity drop position: ne=" << mouse_pos.ne
                                   << " se=" << mouse_pos.se
                                   << " up=" << mouse_pos.up
                                   << " (for spawn_tool: ./spawn_tool <entity> 0 "
                                   << mouse_pos.ne << " " << mouse_pos.se << ")");
        ...
        binding_func_t move_entity{[&](const event_arguments &args,
                ...
                // Log entity move position for spawn_tool debugging
                log::log(MSG(info) << "Entity move position: ne=" << mouse_pos.ne
                                   << " se=" << mouse_pos.se
                                   << " up=" << mouse_pos.up
                                   << " (for spawn_tool: ./spawn_tool <entity> 0 "
                                   << mouse_pos.ne << " " << mouse_pos.se << ")");
        ...
        binding_func_t cycle_spawn_type{[&](const event_arguments &args,
                                            const std::shared_ptr<Controller> controller) {
                auto mouse_pos = args.mouse.to_phys3(camera);
                event::EventHandler::param_map::map_t params{
                        {"cycle", true},
        ...
        binding_action cycle_spawn_type_action{forward_action_t::SEND, cycle_spawn_type};
        Event ev_mouse_rmb_ctrl{
                event_class::MOUSE_BUTTON,
                Qt::MouseButton::RightButton,
                Qt::KeyboardModifier::ControlModifier,
                QEvent::MouseButtonRelease};

        ctx->bind(ev_mouse_rmb_ctrl, cycle_spawn_type_action);
}
